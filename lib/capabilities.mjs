...
import { READ_MODEL_NAMES } from './database.mjs';
...
function toolParameters(tool) {
  ...
  if (tool?.name === 'execute_query') {
    const props = (result.parameters.properties = result.parameters.properties || {});
    props.model = {
      type: 'string',
    ...
    if (!Array.isArray(result.parameters.required)) {
      result.parameters.required = ['model'];
    } else if (!result.parameters.required.includes('model')) {
      result.parameters.required.push('model');
    }
  }
  if (tool && typeof tool === 'object') {
