// Builds the single set of invokable capabilities — workspace tools plus every
// enabled MCP server's declared tools — that a model can call directly during
// conversation. The same registry backs autonomous tool-calling; the existing
// /slash matcher (skills) and @agent dispatcher are separate entry points by
// design (see docs/adr-0002-unified-capability-registry.md) and are not part
// of this registry yet.
...
// capabilities, sourced from every MCP server's declared tools plus the two
// core app tools not already covered by one (read_workspace_file and
...
// tool name with two different execution paths).
