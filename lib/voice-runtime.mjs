...
export class VoiceRuntime {
  constructor({ database, publish, paths, manifest }) {
    this.database = database;
    this.publish = publish;
    this.bootstrap = new VoiceModelBootstrap({ publish, root: paths?.modelRoot, temporaryRoot: paths?.tempRoot, ...(manifest ? { manifest } : {}) });
    this.state = 'bootstrap';
    this.detail = null;
    this.ready = false;
    this.mode = database.setting('voice.mode', 'wake');
  }

  isReady() { return this.ready; }
  async initialize() {
    this.detail = null;
    for (const modelId of ['wake.hey-jarvis', 'stt.whisper-base-en', 'tts.kokoro-v1', 'vad.silero-v6']) {
      try { await this.install(modelId); }
      catch (error) { this.detail = `Unable to install ${modelId}: ${error.message}`; this.publishVoiceState(this.state, { detail: this.detail }); }
    }
    this.ready = (await this.bootstrap.status()).every((model) => model.ready);
    // Acquisition is over either way, so the reported state stops saying models
    // are still being fetched.
    if (this.ready && this.state === 'bootstrap') { this.state = 'ready'; this.publishVoiceState(this.state); }
    return this;
  }
  async status() {
    const detail = this.detail || null; const models = await this.bootstrap.status();
  ...
  message(state = this.state) {
    ...
    if (state === 'returning-to-listen') return 'Recovering the local voice loop before listening again.';
  ...
  // Every voice-state publish carries the same computed `message` that GET /api/voice
  // returns, so a live SSE subscriber never has to re-poll just to keep its presence
  // copy current (see src/hooks/useVoiceStatus.ts).
  publishVoiceState(state, extra = {}) { this.publish({ type: 'voice-state', state, message: this.message(state), ...extra }); }
  async install(modelId) {
    ...
    this.publishVoiceState(this.state);
    return result;
  }
  setState(state, detail = null) { this.state = state; this.database.setSetting('voice.runtime', { state, detail, updatedAt: new Date().toISOString() }); this.publishVoiceState(state, { detail }); }
  setEnabled(enabled) { this.database.setSetting('voice.enabled', Boolean(enabled)); this.publishVoiceState(enabled ? this.state : 'muted', { enabled: Boolean(enabled) }); }
  setMode(mode) { if (!localVoiceModes.includes(mode)) throw new Error(`Unsupported local voice mode: ${mode}.`); this.mode = mode; this.database.setSetting('voice.mode', mode); this.publishVoiceState(this.state, { mode }); }
  setVoice(voice) { if (!localKokoroVoices.includes(voice)) throw new Error(`Voice ${voice} is not installed locally.`); this.database.setSetting('voice.kokoro.voice', voice); this.publishVoiceState(this.state, { voice }); }
  ...
  setSession(conversationId, state, turnId = null) { this.database.setSetting('voice.active-session', { conversationId, turnId, state, updatedAt: new Date().toISOString() }); }
  event(event = {}) {
