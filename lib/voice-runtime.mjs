...
export class VoiceRuntime {
  constructor({ database, publish, paths }) { this.database = database; this.publish = publish; this.bootstrap = new VoiceModelBootstrap({ publish, root: paths?.modelRoot, temporaryRoot: paths?.tempRoot }); this.state = 'bootstrap'; this.detail = null; this.mode = 'wake'; }
  async initialize() {
    this.detail = null;
  ...
  setMode(mode) { if (!localVoiceModes.includes(mode)) throw new Error(`Unsupported local voice mode: ${mode}.`); this.mode = mode; this.publishVoiceState(this.state, { mode }); }
