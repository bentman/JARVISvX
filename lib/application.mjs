...
export function createJarvisApp({ database, events = new AssistantEventHub() } = {}) {
  ...
  const appObj = {
    ...
    // depth-2 delegation cap) — previously built and tested (test/agent-runtime.test.mjs)
    // but never reachable outside the test file. Served at GET/POST /api/agent-bus/*.
    agentBusTools: () => agentRuntime.busMcp.getTools(),
    executeAgentBusTool: (toolName, params) => agentRuntime.busMcp.executeTool(toolName, params),
    ...
    async *chat({ conversationId, content, providerId, userOverrideProvider, model, allowCloud = false, allowToolWrites = false, signal, origin = 'desktop-text' }) {
      ...
      const capabilities = selected.supportsToolCalling ? buildCapabilityRegistry(appObj) : [];
