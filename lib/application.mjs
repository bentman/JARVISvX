...
export function createJarvisApp({ database, events = new AssistantEventHub() } = {}) {
  ...
  const appObj = {
    ...
    // Probes a not-yet-saved provider connection (Providers panel's "Add
    // Provider"/"Edit" form) for its real model list, so the Default Model
    // field can be a live dropdown instead of a free-text guess. Never
    // touches the database — this is a throwaway instance for one health check.
    ...
    // Agent Profiles editor (AgentOrchestrationView.tsx) — createAgent adds a new
    // custom role; updateAgent edits an existing one's adapter/cli/voice/
    // capabilities (built-ins) or full profile (custom); deleteAgent removes a
    // custom agent. All three persist to .jarvis/agents.json (see AgentRegistry).
    ...
    // The real, exhaustive selector options for the Agent Profiles editor — the
    // same constants AgentRegistry validates create/update against, so the UI can
    // never offer a choice the backend would then reject.
    agentEditorOptions: () => ({
    ...
    // Real skills.sh / `skills` CLI integration (see lib/skills-source.mjs) — fetches
    // an actual SKILL.md from GitHub, parses its real frontmatter/instructions, and
    // stores it as a genuine executable custom skill (no fabricated content).
