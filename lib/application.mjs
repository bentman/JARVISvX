...
export function createJarvisApp({ database, events = new AssistantEventHub(), paths = createRuntimePaths(), voiceManifest } = {}) {
  ...
  const appObj = {
    ...
    async initializeCore() { await agentRuntime.initialize(db.roots().map((r) => r.path)); return this; },
    ...
    async *chat({ conversationId, content, providerId, userOverrideProvider, agentProviderId, model, authorization = EMPTY_AUTHORIZATION, signal, origin = 'desktop-text' }) {
      ...
      try {
        ...
        while (true) {
          ...
          for await (const piece of selected.streamChat({ messages: providerMessages, system: systemInstruction || undefined, model: selectedModel, signal: controller.signal, tools: toolSchemas.length ? toolSchemas : undefined })) {
