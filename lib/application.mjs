...
import { getHardwareProfile, pingLocalEndpoint as pingLocal, routeTurn } from './orchestrator.mjs';
import { extractMemoryFactsByRegex, selectMemories } from './memory-engine.mjs';
import { AgentRuntime } from './agents/runtime.mjs';
import { DEFAULT_ROSTERS } from './agents/coordinator.mjs';
...
export function buildSystemInstruction(sections = {}) {
  return INSTRUCTION_SECTIONS
    .filter(([key]) => String(sections[key] || '').trim())
    .map(([key, title]) => `=== ${title} ===\n${String(sections[key]).trim()}`)
    .join('\n\n');
}

export function createJarvisApp({ database, events = new AssistantEventHub(), paths = createRuntimePaths(), voiceManifest } = {}) {
  const db = database || new JarvisDatabase({ dataRoot: paths.dataRoot, dbPath: paths.databasePath });
  const registry = new ProviderRegistry({ database: db }).reload();
  const active = new Map();
  const publish = (event) => events.publish(event);
  const voice = new VoiceRuntime({ database: db, publish, paths, manifest: voiceManifest });
  const agentRuntime = new AgentRuntime({ database: db, selectProvider: (options) => appObj.selectProvider(options), publish, agentConfigPath: paths.agentConfigPath });
  const grants = new GrantLedger({ database: db });

  const appObj = {
    db, registry, events, voice, agentRuntime, grants, paths,
    // The /api/providers health response consumes the flat .providers view.
    get providers() { return registry.list(); },

    // Core services gate readiness. Voice-model acquisition can take minutes on a
    // cold start, so it runs alongside rather than in front of it.
    async initializeCore() { await agentRuntime.initialize(); return this; },
    startVoiceBootstrap() { return voice.initialize().catch((error) => { voice.detail = error.message; return voice; }); },
    async initialize() { await appObj.initializeCore(); await appObj.startVoiceBootstrap(); return this; },

    getProvider(id) {
      if (!id) return registry.getDefault();
      const p = registry.get(id);
      if (p) return p;
      throw registry.status(id) === 'disabled'
        ? new ProviderError(`Provider '${id}' is disabled.`, 'provider_disabled')
        : new ProviderError(`Provider '${id}' is not a configured provider.`, 'unknown_provider');
    },
    ...
    async resolveModel({ provider, requested = null }) {
      const candidates = [requested, db.setting(`provider.model.${provider.id}`, null), provider.model];
      for (const candidate of candidates) {
        if (String(candidate || '').trim()) return String(candidate).trim();
      }
      const discovered = (await provider.listModels().catch(() => []))[0];
      if (String(discovered || '').trim()) return String(discovered).trim();
      throw new ProviderError(`Select a model for ${provider.label}.`, 'model_required');
    },
    selectProvider({ text = '', providerId = null, agentProviderId = null, authorization = EMPTY_AUTHORIZATION } = {}) {
      const orch = db.orchestrationSettings();
      return routeTurn(text, {
        mode: orch.mode,
        userOverrideProvider: providerId,
        agentProviderOverride: agentProviderId,
        allowCloud: hasAnyGrant(authorization, ACTIONS.CLOUD),
    ...
    async generate({ system, prompt, model, providerId, authorization = EMPTY_AUTHORIZATION }) {
      ...
      const selectedModel = await appObj.resolveModel({ provider, requested: model });
      let output = '';
      for await (const piece of provider.streamChat({ messages: [{ role: 'user', content: prompt }], system: system || undefined, model: selectedModel })) {
    ...
    agentEditorOptions: () => ({
      ...
      defaultRosters: DEFAULT_ROSTERS,
    }),
    startAgentRun: ({ authorization = EMPTY_AUTHORIZATION, ...options } = {}) => agentRuntime.startRun({ ...options, authorization }).run,
    ...
    // A probe result is persisted so the reported health is the one that was measured.
    async pingMcpServer(id) {
      const server = db.mcpServer(id);
      if (!server) throw new ProviderError('MCP Server not found.', 'not_found');
      const probe = await pingMcp(server, appObj);
      db.recordMcpProbe(id, probe);
      return probe;
    },
    ...
    async *chat({ conversationId, content, providerId, userOverrideProvider, agentProviderId, model, authorization = EMPTY_AUTHORIZATION, signal, origin = 'desktop-text' }) {
      ...
      // One selection, before any message is created or provider metadata is read.
      const selection = appObj.selectProvider({
        text,
        providerId: userOverrideProvider || providerId || null,
        agentProviderId: agentProviderId || null,
        authorization,
      });
      if (!selection.provider) throw new ProviderError(selection.reason, selection.code);
      ...
      const selectedModel = await appObj.resolveModel({ provider: selected, requested: model });
      ...
      let output = '';
      yield { type: 'start', conversationId: conversation.id, turnId, provider: selected.id, model: selectedModel, routing: { source: selection.source, reason: selection.reason } };
      publish({ type: 'session', state: 'turn-start', conversationId: conversation.id, turnId, origin });
      ...
      // Memory reaches every protocol through the canonical instruction; the
      // capability section only describes tools an adapter can actually receive.
      const systemInstruction = buildSystemInstruction({
        instructions: db.setting('assistant.instructions', ''),
        memory: selectMemories(db).text,
      ...
      try {
        ...
        while (true) {
          ...
          const allowTools = round < MAX_TOOL_ROUNDS && toolSchemas.length > 0;
          for await (const piece of selected.streamChat({
            ...
            if (piece && typeof piece === 'object' && piece.type === 'tool_call') {
              if (allowTools) pendingToolCalls.push(piece);
              continue;
            }
          ...
          for (const call of pendingToolCalls) {
            ...
            executedTools.push({ name: tool.name, arguments: call.arguments, output: resultText });

            const remaining = MAX_TOOL_ROUNDS - round;
            const budgetNotice = remaining > 0
              ? `\n[Remaining tool rounds: ${remaining}]`
              : '\n[Tool round budget reached. Synthesize final answer from gathered results.]';
            providerMessages.push({ role: 'tool', toolCallId: call.id, content: `${resultText}${budgetNotice}` });
          }
        }
        yield* emitPieces(splitter.flush());
        if (!output.trim() && executedTools.length) {
          const fallback = executedTools.map((t) => `Ran \`${t.name}\`:\n\`\`\`\n${t.output}\n\`\`\``).join('\n\n');
          yield* emitPieces([{ type: 'content', text: fallback }]);
        }
        if (output) db.addMessage(conversation.id, 'assistant', output, selected.id, 'complete', origin);
    ...
    // The effective selection is what an unpinned turn would route to right now.
    settings() {
      const orch = db.orchestrationSettings();
      const selection = routeTurn('', { mode: orch.mode, allowCloud: registry.getByTags(['cloud']).length > 0 }, registry);
      const active = selection.provider || null;
      return {
        activeProvider: active?.id || null,
        activeModel: active ? this.modelFor(active.id) : null,
        activeProviderLabel: active?.label || null,
        isCloudProvider: Boolean(active?.tags?.includes('cloud')),
        effectiveSource: selection.source || null,
        unavailableReason: active ? null : selection.reason,
        models: Object.fromEntries(registry.list().map((provider) => [provider.id, this.modelFor(provider.id)])),
    ...
    // The write happens first: a failure leaves the edit reviewable.
    async approveWorkspaceEdit(id) {
      const edit = db.workspaceEdit(id);
      if (!edit) throw new ProviderError(`Workspace edit "${id}" not found.`, 'not_found');
      if (edit.status !== 'pending_review') throw new ProviderError(`Workspace edit "${id}" is already ${edit.status}.`, 'conflict');
      await writeWorkspaceFile(edit.file_path, edit.content, db.roots().map((root) => root.path));
