...
export class DaemonClient {
  ...
  // `paths` names the runtime set whose discovery record identifies the daemon,
  // so a caller that resolved its own set is not answered by another one.
  static async connect({ start = true, paths = createRuntimePaths() } = {}) {
    let discovery = await readDiscovery(paths);
    if (!discovery || !(await healthy(discovery))) {
      if (!start) throw new Error('JARVIS daemon is not running.');
      await startDaemon().catch(() => {});
      discovery = await waitForDiscovery(paths);
    }
  ...
  async runAgent(payload, { wait = true, pollIntervalMs = 500, signal } = {}) {
    const started = await this.json('/agents/run', { method: 'POST', body: JSON.stringify(payload) });
    if (!wait || !started?.id || started.status === 'completed' || started.status === 'failed') {
      return started;
    }
    return this.waitForRun(started.id, { pollIntervalMs, signal });
  }
  agentRun(id) { return this.json(`/runs/${id}`); }
  agentRuns(conversationId = null) {
    const qs = conversationId ? `?conversationId=${encodeURIComponent(conversationId)}` : '';
    return this.json(`/runs${qs}`);
  }
  async waitForRun(id, { pollIntervalMs = 500, signal } = {}) {
    while (!signal?.aborted) {
      const run = await this.agentRun(id);
      if (run.status === 'completed' || run.status === 'failed') return run;
      await new Promise((resolve) => setTimeout(resolve, pollIntervalMs));
    }
    throw new Error('Wait for agent run aborted');
  }
...
async function readDiscovery(paths = createRuntimePaths()) { try { return JSON.parse(await fs.readFile(paths.discoveryPath, 'utf8')); } catch { return null; } }
// The ownership probe answers in every lifecycle state and reads no provider or
// hardware state, so it reports whether a daemon is there. `/api/health` runs the
// full diagnostic sweep and cannot answer inside a liveness budget.
async function healthy(discovery) { try { const response = await fetch(`http://127.0.0.1:${discovery.port}/daemon/status`, { headers: { 'x-jarvis-token': discovery.token }, signal: AbortSignal.timeout(2000) }); return response.ok; } catch { return false; } }
...
async function waitForDiscovery(paths = createRuntimePaths()) {
