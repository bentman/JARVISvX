...
export async function getHardwareProfile(providers = []) {
  ...
  // A local server is reported only when a configured local provider answered a
  // health probe; throughput is reported only when something measured it.
  const localHealth = (info?.providers || []).find((provider) => provider.tags?.includes('local'));
  const localProvider = providers.find((provider) => provider.id === localHealth?.id);

  return {
    cpuCores: os.cpus().length || null,
    ramGB: memoryGB,
    freeRamGB,
    gpuName: info?.acceleration?.gpus?.[0]?.name || null,
    os: `${os.type()} ${os.arch()} (${os.release()})`,
    accelerationStatus: info?.acceleration?.status || 'unknown',
    recommendedLocalModel,
    isLocalServerDetected: localHealth ? Boolean(localHealth.available) : false,
    localServerUrl: localProvider?.baseUrl || null,
    localTokensPerSec: null
  };
}
...
const SEARCH_TRIGGER_PATTERN = /\b(search|google|news|weather|today|latest|breakthrough)\b/i;
const CODING_TRIGGER_PATTERN = /\b(code|function|typescript|react|python|algorithm|bug|fix|refactor|macro)\b/i;
...
export function routeTurn(content, context, registry) {
  ...
  const requiresSearch = autoEscalateRules.requireSearch && SEARCH_TRIGGER_PATTERN.test(text);
  const requiresCoding = autoEscalateRules.requireCodeExecution && CODING_TRIGGER_PATTERN.test(text);
