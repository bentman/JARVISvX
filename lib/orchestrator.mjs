...
export async function pingLocalEndpoint(endpointUrl) {
  ...
  const targetUrl = url.endsWith('/models') || url.endsWith('/v1') ? (url.endsWith('/v1') ? `${url}/models` : url) : `${url}/models`;
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      const models = Array.isArray(data.data) ? data.data.map((m) => m.id) : ['Llama-3.2-3B-Instruct', 'Llama-3.2-1B-Instruct'];
      return {
        status: 'connected',
        latencyMs,
        endpoint: url,
