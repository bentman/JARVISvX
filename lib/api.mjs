...
export function createApiRouter(jarvis, { token } = {}) {
  const router = Router();
  router.get('/voice-assets/:model/*splat', (req, res) => { const tail = Array.isArray(req.params.splat) ? req.params.splat.join('/') : String(req.params.splat || ''); const file = jarvis.voice.bootstrap.file(req.params.model, tail); if (!file) return res.status(404).end(); res.sendFile(file); });
  ...
  router.post('/chat/:id/cancel', (req, res) => res.json({ cancelled: jarvis.cancel(req.params.id) }));
