...
export function createApiRouter(jarvis, { token, lifecycle = () => 'ready' } = {}) {
  ...
  router.post('/agents/run', requireReady, async (req, res, next) => { try { const { approvals, ...options } = req.body; res.json(await jarvis.executeAgentRun({ ...options, authorization: jarvis.authorizationFor(approvals, 'api') })); } catch (error) { next(error); } });
