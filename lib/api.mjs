...
export function createApiRouter(jarvis, { token } = {}) {
  ...
  // Changing JARVIS_DATA_DIR requires an app restart; this endpoint lets the
  // UI display the active path the same way it displays workspace roots.
  ...
  router.post('/mcp', (req, res, next) => { try { res.status(201).json(jarvis.addMcpServer(req.body)); } catch (error) { next(error); } });
