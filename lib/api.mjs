...
import { API_ERROR_STATUS } from './contracts.mjs';
import { dataDirectoryInfo } from './data-migration.mjs';


const sse = (res, event) => res.write(`event: ${event.type}\ndata: ${JSON.stringify(event)}\n\n`);

// A delete or toggle of something that is not there is a 404, not a 200 saying
// nothing was removed.
const LOOPBACK_ADDRESSES = new Set(['127.0.0.1', '::1']);

const isDaemonHost = (host, port) => {
  if (host === undefined) return true;  // a same-origin fetch may send no Origin
  const [name, hostPort] = String(host || '').split(':');
  return LOOPBACK_ADDRESSES.has(name) || name === 'localhost' ? Number(hostPort || 80) === port : false;
};
...
const removed = (res, wasRemoved, what) => (wasRemoved ? res.json({ removed: true }) : res.status(404).json({ error: `${what} not found.`, code: 'not_found' }));

export function createApiRouter(jarvis, { token, lifecycle = () => 'ready' } = {}) {
  const router = Router();
  // Readiness gates work submission. Ownership liveness is a separate question,
  // answered by /daemon/status.
  const requireReady = (req, res, next) => (lifecycle() === 'starting'
    ? res.status(503).json({ error: 'The daemon is still starting.', code: 'not_ready' })
    : next());
  ...
  // The browser-hosted UI bootstrap. It hands the daemon token to a page served
  // by this daemon over loopback; Electron and the CLI use their own bridges.
  router.get('/session', (req, res) => {
    const port = req.socket?.localPort || 3210;
    const address = req.socket?.remoteAddress || '';
    if (!LOOPBACK_ADDRESSES.has(address.replace(/^::ffff:/, ''))) return res.status(403).json({ error: 'The session bootstrap is available over loopback only.', code: 'policy_denied' });
    if (!isDaemonHost(req.get('host'), port) || !isDaemonHost(originHost(req.get('origin')), port)) {
      return res.status(403).json({ error: 'The session bootstrap requires the daemon origin.', code: 'policy_denied' });
    }
    res.set('cache-control', 'no-store');
    res.json({ token, port, status: 'authenticated' });
  });
  ...
  router.get('/health', async (_req, res, next) => {
    try {
      const health = await jarvis.health();
      const voice = await jarvis.voice.status();
      const degraded = voice.state === 'bootstrap' || !jarvis.voice.isReady()
        ? [{ subsystem: 'voice', reason: voice.detail || 'Voice models are still being acquired.', recovery: 'Text operation is unaffected; voice becomes ready once model acquisition completes.' }]
        : [];
      res.json({ ...health, lifecycle: lifecycle(), degraded });
  ...
  router.get('/providers', async (_req, res, next) => { try { res.json({ settings: jarvis.settings(), providers: await jarvis.registry.health() }); } catch (error) { next(error); } });
  ...
  router.delete('/conversations/:id', (req, res) => removed(res, jarvis.deleteConversation(req.params.id), 'Conversation'));
  ...
  router.post('/agents/run', requireReady, async (req, res, next) => {
    try {
      const { approvals, wait = false, ...options } = req.body;
      const authorization = jarvis.authorizationFor(approvals, 'api');
      if (wait) {
        res.json(await jarvis.executeAgentRun({ ...options, authorization }));
      } else {
        const run = await jarvis.startAgentRun({ ...options, authorization });
  ...
  router.get('/runs/:id', (req, res) => {
    const item = jarvis.agentRun(req.params.id);
    if (!item) return res.status(404).json({ error: 'Agent run not found.' });
    res.json(item);
  });
  ...
  router.post('/chat', requireReady, async (req, res, next) => {
  ...
  router.delete('/workspace-roots/:id', (req, res) => removed(res, jarvis.removeRoot(req.params.id), 'Workspace root'));
  ...
  router.delete('/provider-registry/:id', (req, res) => removed(res, jarvis.deleteProvider(req.params.id), 'Provider'));
  ...
  router.put('/memory/:id', (req, res, next) => { try { const memory = jarvis.updateMemory(req.params.id, req.body); if (!memory) return res.status(404).json({ error: 'Memory not found.', code: 'not_found' }); res.json(memory); } catch (error) { next(error); } });
  router.delete('/memory/:id', (req, res) => removed(res, jarvis.deleteMemory(req.params.id), 'Memory'));
  ...
  router.delete('/mcp/:id', (req, res) => removed(res, jarvis.deleteMcpServer(req.params.id), 'MCP server'));
  ...
  router.put('/skills/:id', (req, res, next) => { try { const skill = jarvis.updateSkill(req.params.id, req.body); if (!skill) return res.status(404).json({ error: 'Skill not found.', code: 'not_found' }); res.json(skill); } catch (error) { next(error); } });
  router.delete('/skills/:id', (req, res) => removed(res, jarvis.deleteSkill(req.params.id), 'Skill'));
  router.post('/skills/:id/toggle', (req, res, next) => { try { const skill = jarvis.toggleSkill(req.params.id); if (!skill) return res.status(404).json({ error: 'Skill not found.', code: 'not_found' }); res.json(skill); } catch (error) { next(error); } });
  ...
  router.use((error, _req, res, _next) => res.status(API_ERROR_STATUS[error.code] || 400).json({ error: error.message, code: error.code || 'error' }));
  return router;
}
