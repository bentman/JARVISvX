...
async function probeWindowsAcceleration() {
  ...
  return {
    status: gpus.length ? 'available' : 'unavailable',
    gpus,
    npu,
    reason: gpus.length ? undefined : (adapterResult.error?.message || 'No display adapter was reported.')
  };
}
...
async function probeLinuxAcceleration() {
  ...
  return {
    status: gpus.length ? 'available' : 'unavailable',
    gpus,
    npu: { status: 'unknown', reason: 'No Linux NPU probe is implemented.' },
    reason: gpus.length ? undefined : 'No NVIDIA device and no DRM card were reported.'
  };
}

async function probeDrmCards() {
  const cards = await fs.readdir('/sys/class/drm').catch(() => []);
  const found = [];
