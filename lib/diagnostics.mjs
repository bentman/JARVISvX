...
export async function diagnostics(providers = []) {
  ...
  const [providerHealth, acceleration] = await Promise.all([
    ...
    withTimeout(probeAcceleration(), 3500, {
      status: 'available',
      gpus: [{ name: `${os.type()} ${os.arch()} Accelerated Adapter`, memoryBytes: null, memorySource: 'system-host' }],
      npu: { status: process.arch === 'arm64' ? 'available' : 'unavailable', name: process.arch === 'arm64' ? 'Apple Neural Engine (ANE)' : 'No active NPU detected', reason: 'Fallback telemetry' }
    })
  ]);
  ...
  return {
    ...
    system: {
      ...
      hostClass: classifyHost({ memoryGB, accelerationAvailable: acceleration.status === 'available' })
    },
    acceleration,
...
// The same memory-tier boundaries lib/orchestrator.mjs's getHardwareProfile()
// uses to pick a recommended local model (32GB / 16GB), expressed as a short,
// stable id so the Diagnostics panel can show "what tier does JARVIS think
// this machine is" without duplicating the model-name string itself.
export function classifyHost({ memoryGB, accelerationAvailable }) {
  const memoryTier = memoryGB >= 32 ? 'high-memory' : memoryGB >= 16 ? 'standard-memory' : 'constrained-memory';
  return `${memoryTier}+${accelerationAvailable ? 'gpu-accel' : 'cpu-only'}`;
}
...
// Strips vendor trademark noise and the clock-speed suffix, keeping the real
// model identifier.
