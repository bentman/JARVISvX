...
// A raced promise is abandoned, not stopped. Where the work accepts a signal the
// budget aborts it, so the limit is the real cost of the probe.
const withTimeout = (promise, ms, fallback, controller) =>
  Promise.race([
    promise,
    new Promise((resolve) => setTimeout(() => { controller?.abort(); resolve(fallback); }, ms))
  ]);

const ACCELERATION_FRESHNESS_MS = 30_000;
let cachedAcceleration = null;
let cachedAccelerationAt = 0;
let inFlightAcceleration = null;

async function getAcceleration() {
  if (cachedAcceleration && Date.now() - cachedAccelerationAt < ACCELERATION_FRESHNESS_MS) {
    return cachedAcceleration;
  }
  if (inFlightAcceleration) return inFlightAcceleration;
  inFlightAcceleration = withTimeout(probeAcceleration(), 3500, {
    status: 'unknown',
    gpus: [],
    ...
    cachedAcceleration = measured;
    cachedAccelerationAt = Date.now();
    inFlightAcceleration = null;
    return measured;
  }).catch((error) => {
    inFlightAcceleration = null;
    throw error;
  });
  return inFlightAcceleration;
}
  ...
  const safeHealth = async (provider) => {
    ...
    try {
      return await withTimeout(
        provider.health(controller.signal),
        1500,
...
function classifyHost({ platform = process.platform, arch = process.arch, acceleration = {} } = {}) {
...
function mergeGpuInventory(adapters, reports) {
