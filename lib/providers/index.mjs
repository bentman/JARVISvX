...
export class ProviderRegistry {
  ...
  /**
   * Instantiate a one-off provider from raw connection fields, not a saved DB
   * row — used to probe a provider's real model list (Providers panel's "Add
   * Provider" form) before anything has been saved. No `database` dependency,
   * so this also works from a bare `new ProviderRegistry()` with no args.
   */
