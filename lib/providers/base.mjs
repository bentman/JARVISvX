...
export class BaseProvider {
  constructor({ id, label, baseUrl, model = '', apiKey = null, tags = [], priority = 50 }) {
    Object.assign(this, { id, label, baseUrl: baseUrl?.replace(/\/$/, ''), model, apiKey, tags, priority });
  }
  ...
  async health() {
    try {
      ...
      return { id: this.id, label: this.label, available: true, models, tags: this.tags, priority: this.priority };
    } catch (error) {
      return { id: this.id, label: this.label, available: false, reason: error.message, models: [], tags: this.tags, priority: this.priority };
    }
  }
