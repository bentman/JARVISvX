...
export async function startDaemon({ port = Number(process.env.JARVIS_PORT || 3210), token = crypto.randomBytes(32).toString('hex') } = {}) {
  ...
  const lock = await acquireLock(); const jarvis = createJarvisApp(); await jarvis.initialize();
...
async function acquireLock() { const lock = lockPath(); try { return await fsp.open(lock, 'wx'); } catch (error) { if (error.code !== 'EEXIST') throw error; const existing = await daemonDiscovery(); if (existing && await alive(existing)) throw new Error('JARVIS daemon is already running.'); await fsp.unlink(lock).catch(() => {}); return fsp.open(lock, 'wx'); } }
async function alive({ port, token }) { try { const response = await fetch(`http://127.0.0.1:${port}/api/health`, { headers: { 'x-jarvis-token': token }, signal: AbortSignal.timeout(500) }); return response.ok; } catch { return false; } }
