...
// The project-local default data root (always <root>/data, never a sub-directory,
// and never cwd-relative — the daemon can be launched via the globally-linked
// `jarvis` CLI from any working directory).
const defaultDataRoot = path.join(root, 'data');

export async function startDaemon({ port = Number(process.env.JARVIS_PORT || 3210), token = crypto.randomBytes(32).toString('hex') } = {}) {
  // Run migration before anything touches the data directory.
  // In daemon/CLI context this is always non-interactive; the safe 'import'
  // (merge) strategy is used — never destructive without user confirmation.
  await migrateDataDirectory(defaultDataRoot, dataDirectory());
