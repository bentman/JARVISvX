...
export async function startDaemon({ port = Number(process.env.JARVIS_PORT || 3210), token = crypto.randomBytes(32).toString('hex'), paths = createRuntimePaths(), onMigrationConflict, voiceManifest } = {}) {
  ...
  try {
    ...
    const close = async () => { await teardown(); await fsp.unlink(paths.lockPath).catch(() => {}); };
