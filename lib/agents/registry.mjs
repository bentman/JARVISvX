...
import { PROJECT_ROOT, createRuntimePaths } from '../runtime-paths.mjs';
...
// Fields a profile loaded from a workspace may set. Command paths, process
// arguments, and anything else outside this list are rejected.
const WORKSPACE_SCOPED_FIELDS = new Set(['id', 'name', 'description', 'adapter', 'cli', 'voice', 'capabilities', 'instructions', 'provider']);
const APPLICATION_SCOPED_FIELDS = new Set([...WORKSPACE_SCOPED_FIELDS, 'command']);
...
export class AgentRegistry {
  ...
  // the registry. Workspace scope customizes identity and capabilities; the
  // application-owned source is authoritative for executable wiring.
  validateProfile(id, agent, { filePath, applicationOwned }) {
    const allowed = applicationOwned ? APPLICATION_SCOPED_FIELDS : WORKSPACE_SCOPED_FIELDS;
    const unknown = Object.keys(agent).filter((key) => !allowed.has(key));
    ...
    const cli = agent.cli || (applicationOwned ? agent.command : null) || 'claude';
    ...
    if (applicationOwned && agent.command !== undefined && agent.command !== command) {
  ...
  async load(roots = []) {
    this.profiles = new Map(Object.entries(DEFAULT_AGENT_PROFILES));
    this.rejections = [];
    await this.adoptSeededOverrides();
    const sources = roots.map((rootPath) => ({ filePath: path.join(rootPath, '.jarvis', 'agents.json'), applicationOwned: false }));
    sources.push({ filePath: this.configPath(), applicationOwned: true });

    for (const { filePath, applicationOwned } of sources) {
      let parsed;
      try {
        parsed = JSON.parse(await fs.readFile(filePath, 'utf8'));
      } catch {
        continue;  // Missing or unreadable config falls back to defaults.
      }
      if (!parsed || typeof parsed.agents !== 'object' || parsed.agents === null) continue;
      for (const [id, agent] of Object.entries(parsed.agents)) {
        try {
          this.profiles.set(id, this.validateProfile(id, agent || {}, { filePath, applicationOwned }));
        } catch (error) {
  ...
  // Overrides seeded beside the source tree are adopted once, so an installation
  // that predates the runtime location keeps its saved profiles.
  async adoptSeededOverrides() {
    if (await fs.access(this.agentConfigPath).then(() => true, () => false)) return;
    const seedPath = path.join(PROJECT_ROOT, '.jarvis', 'agents.json');
    let parsed;
    try { parsed = JSON.parse(await fs.readFile(seedPath, 'utf8')); } catch { return; }
    if (!parsed || typeof parsed.agents !== 'object' || parsed.agents === null) return;

    const agents = {};
    for (const [id, agent] of Object.entries(parsed.agents)) {
      try { agents[id] = this.validateProfile(id, agent || {}, { filePath: seedPath, applicationOwned: true }); }
      catch (error) { this.rejections.push({ filePath: seedPath, profileId: id, reason: error.message }); }
    }
    await this.writeConfigFile({ ...parsed, agents });
  }
