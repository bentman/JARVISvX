...
// Deliberately exempt from the cloud-approval gate that ProcessAdapter enforces
// (lib/agents/adapters/process.mjs) and that chat() enforces (lib/application.mjs).
// This adapter never touches JARVIS's own provider registry at all — it shells out to
// external CLIs (claude, codex, copilot, cline, agy) that authenticate and bill
// entirely on their own, outside anything JARVIS tracks as a "cloud provider." There is
// no JARVIS-registered cloud request here to gate: the analogous approval decision (if
// any) belongs to whatever credentials/config each external CLI itself uses, not to
// this adapter. See docs/tech-debt-fragmentation-audit.md, Finding 6.
export class AcpAdapter {
  ...
  // Writes a follow-up message to a still-running run's stdin. Only
  // meaningful for the standard ACP protocol mode (npx @agentclientprotocol/cli),
  // whose stdin is piped for a real bidirectional session — the fixed CLIs
  // (claude/codex/copilot/cline/agy) are invoked in single-shot print/exec
  // mode with no stdin attached (see buildCliArgs and the spawn() call
  // above), so there is no live session to send to for those, and this
  // returns an honest failure rather than a fake delivery.
  send(runId, message) {
    const child = this.runs.get(runId);
