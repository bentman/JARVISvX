...
export class RunCoordinator {
  ...
  async invokeAgent({
    ...
    const speaker = { name: agent.name, voice: agent.voice, id: agent.id };
    this.publish({
      type: 'agent-start',
      runId,
      agentId: agent.id,
    ...
    for await (const event of adapter.invoke({
      ...
      if (event.type === 'token') {
        const tokenSpeaker = event.speaker ? { ...speaker, ...event.speaker } : speaker;
        for (const piece of splitter.push(event.value)) {
  ...
  startRun({
    ...
    return { run: runRecord, promise };
  }

  async executeRun(options) {
    const { promise } = this.startRun(options);
    return promise;
  }
}
