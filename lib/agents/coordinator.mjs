export class RunCoordinator {
  ...
  authorizeAgent(agent, requestedCapabilities, approved) {
    const policyEvaluation = this.policy.evaluate({ agent, requestedCapabilities, approved });
    if (!policyEvaluation.allowed) {
      throw new Error(`Policy Rejected: ${policyEvaluation.reason}`);
    }
  ...
  async invokeAgent({
    ...
    this.authorizeAgent(agent, requestedCapabilities, approved);
    ...
    for await (const event of adapter.invoke({
      ...
      if (event.type === 'failed') {
        throw new Error(event.error || `Agent '${agent.id}' failed.`);
      }
      if (event.type === 'token') {
        output += event.value;
        this.publish({
