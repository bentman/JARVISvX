...
export class UnauthorizedRelocationError extends Error {
  constructor(source, target) {
    super(`JARVIS_DATA_DIR would move the data directory from ${source} to ${target}. Re-run once with JARVIS_DATA_MIGRATE=1 to authorize the move, or clear JARVIS_DATA_DIR to keep using ${source}.`);
    this.name = 'UnauthorizedRelocationError';
    this.code = 'unauthorized_relocation';
    this.source = source;
    this.target = target;
  }
}
...
/**
 * Whether starting against `targetDir` would move existing data out of
 * `sourceDir`. False when the two resolve to the same place, or when the source
 * holds nothing to move — those starts create or adopt a directory rather than
 * relocating one.
 */
export async function wouldRelocate(sourceDir, targetDir) {
  const src = path.resolve(sourceDir);
  if (src === path.resolve(targetDir)) return false;
  return dirHasContents(src);
}
