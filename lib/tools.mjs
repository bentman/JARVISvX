...
const SEARCH_SKIP_DIRS = new Set(['node_modules', '.git', 'dist', 'build', '.next', 'target', '__pycache__', '.venv', 'venv', '.cache', 'electron', 'electron-profile', 'sql-db']);
...
// and scanned-file count. Honors .agentignore and ignores application runtime state.
export async function searchWorkspace(query, roots) {
  ...
  for (const root of approvedRoots) {
    ...
    const ignorePatterns = await loadAgentIgnore(root);
