...
export async function executeMcpTool(server, toolName, params = {}, app) {
  ...
  try {
    ...
    // Real MCP-over-stdio: spawns the server's command, performs the
    // initialize/tools-call handshake, and returns its actual reply (see
    // lib/mcp-stdio.mjs). server.endpoint is the shell command to run.
    if (server.type === 'stdio') {
