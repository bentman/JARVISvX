...
// Built-in /search and /code skill implementations (see seed() and
// upgradeBuiltInSkills() below). Kept as named constants so a fresh
// install's seed data and an existing install's migration read from the
// same source, and so the migration's exact-match check has something
...
CREATE TABLE IF NOT EXISTS providers (id TEXT PRIMARY KEY, name TEXT NOT NULL, protocol TEXT NOT NULL, base_url TEXT NOT NULL, model TEXT NOT NULL DEFAULT '', api_key_enc TEXT, tags TEXT NOT NULL DEFAULT '[]', enabled INTEGER NOT NULL DEFAULT 1, priority INTEGER NOT NULL DEFAULT 50, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);`);
  ...
  // The /search and /code built-in skills originally shipped with stub
  // implementations (a canned string, not a real search or a real
  // generation). Installs that seeded before the real implementations
  // existed already have those old rows in their database; seed() only
  // inserts when the skills table is empty, so it never reaches them. This
  // replaces a built-in skill's code only when it still exactly matches the
  // old stub — if the user edited it, the row no longer matches and is left
  // alone, so a customization is never overwritten.
