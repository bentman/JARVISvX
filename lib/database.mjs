...
const query = (db, sql) => (db.stmt ? db.stmt(sql) : db.prepare(sql));

export const READ_MODELS = {
  conversation_counts: (db) => query(db, 'SELECT (SELECT COUNT(*) FROM conversations) AS conversations, (SELECT COUNT(*) FROM messages) AS messages').all(),
  recent_conversations: (db, limit) => query(db, 'SELECT id, title, created_at, updated_at FROM conversations ORDER BY updated_at DESC, id DESC LIMIT ?').all(limit),
  skill_summary: (db, limit) => query(db, 'SELECT id, name, slash_command, description, enabled, type, version, execution_provenance FROM skills ORDER BY type DESC, created_at DESC, id DESC LIMIT ?').all(limit),
  memory_summary: (db, limit) => query(db, 'SELECT id, category, key, value, importance, updated_at FROM memories ORDER BY updated_at DESC, id DESC LIMIT ?').all(limit),
  agent_run_summary: (db, limit) => query(db, 'SELECT id, agent_id, adapter, mode, status, effective_capabilities, started_at, completed_at FROM agent_runs ORDER BY started_at DESC, id DESC LIMIT ?').all(limit),
  mcp_server_summary: (db, limit) => query(db, 'SELECT id, name, type, status, latency_ms FROM mcp_servers ORDER BY created_at, id LIMIT ?').all(limit),
  workspace_roots: (db, limit) => query(db, 'SELECT id, path, added_at FROM workspace_roots ORDER BY added_at, id LIMIT ?').all(limit),
};

export const READ_MODEL_NAMES = Object.freeze(Object.keys(READ_MODELS));

const toolsCache = new Map();
function parseToolsJson(json) {
  if (!json || json === '[]') return [];
  let parsed = toolsCache.get(json);
  if (!parsed) {
    try { parsed = JSON.parse(json); } catch { parsed = []; }
    toolsCache.set(json, parsed);
  }
  return parsed;
}
...
// Write-ahead logging is what makes `synchronous = NORMAL` retain durability
// across a process crash. A location that cannot provide it — a network file
// system has no shared memory to host the log — is refused rather than opened
// with relaxed durability and no log to recover from.
export class UnsupportedStorageError extends Error {
  constructor(dbPath, journalMode) {
    super(`${dbPath} cannot use write-ahead logging (journal_mode=${journalMode}). Point JARVIS_DATA_DIR at local storage — a network file system is not a supported data root.`);
    this.name = 'UnsupportedStorageError';
    this.code = 'unsupported_storage';
    this.dbPath = dbPath;
...
export class JarvisDatabase {
  ...
  constructor(options = {}) {
    ...
    // journal_mode is a property of the file and persists; synchronous is a
    // property of the connection and is reapplied here on every open — and only
    // once WAL is confirmed, because WAL is what carries the relaxed setting.
    const { journal_mode: journalMode } = this.db.prepare('PRAGMA journal_mode = WAL').get();
    if (journalMode !== 'wal') {
      this.db.close();
      throw new UnsupportedStorageError(this.dbPath, journalMode);
    }
...
ALTER TABLE agent_runs_related RENAME TO agent_runs;`);
      ...
      const defaultMcp = [
        ...
        },
        {
          ...
          tools_json: JSON.stringify([
            {
              name: 'execute_query',
              description: 'Reads one named application read model. Parameters: model (a read-model name), limit (optional row cap).',
              parameters: 'model: string, limit?: string',
              readOnly: true,
              inputSchema: {
                type: 'object',
                properties: {
                  model: {
                    type: 'string',
                    description: `Named application read model. Allowed values: ${READ_MODEL_NAMES.join(', ')}`,
                    enum: READ_MODEL_NAMES
                  },
                  limit: {
                    type: 'integer',
                    description: 'Optional row cap (1 to 200, default 50).'
                  }
                },
                required: ['model']
              }
  ...
  stmt(sql) {
    let statement = this._stmts.get(sql);
    if (!statement) {
      statement = this.db.prepare(sql);
      this._stmts.set(sql, statement);
    }
    return statement;
  }
  ...
  setting(key, fallback = null) {
    ...
    const row = this.stmt('SELECT value FROM settings WHERE key = ?').get(key);
  ...
  setSetting(key, value) {
    this.stmt('INSERT INTO settings(key,value,updated_at) VALUES(?,?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at').run(key, JSON.stringify(value), now());
  ...
  readModel(name, params = {}) {
    ...
    return model(this, boundedLimit(params.limit));
  }

  issueGrant({ id, action, target, requested, expiresAt }) {
    const issuedAt = now();
    this.stmt('INSERT INTO authorization_grants(id,action,target,requested,issued_at,expires_at,consumed_at) VALUES(?,?,?,?,?,?,NULL)')
  ...
  consumeGrant(id, at) {
    const changes = this.stmt('UPDATE authorization_grants SET consumed_at=? WHERE id=? AND consumed_at IS NULL AND expires_at > ?').run(at, id, at).changes;
    if (changes !== 1) return null;
    return this.stmt('SELECT id, action, target, requested, issued_at, expires_at, consumed_at FROM authorization_grants WHERE id=?').get(id);
  }
  recordAuthorization({ id, turnId, origin, action, requestedTarget, grantedTarget, effectiveTarget, outcome }) {
    this.stmt('INSERT INTO authorization_audit(id,turn_id,origin,action,requested_target,granted_target,effective_target,outcome,created_at) VALUES(?,?,?,?,?,?,?,?,?)')
      .run(id, turnId, origin, action, requestedTarget, grantedTarget, effectiveTarget, outcome, now());
  }
  authorizationAudit(limit = 100) {
    return this.stmt('SELECT * FROM authorization_audit ORDER BY created_at DESC, id DESC LIMIT ?').all(limit);
  }
  conversations() { return this.stmt('SELECT * FROM conversations ORDER BY updated_at DESC, id DESC').all(); }
  conversation(id) { return this.stmt('SELECT * FROM conversations WHERE id=?').get(id); }
  messages(conversationId) { return this.stmt('SELECT * FROM messages WHERE conversation_id=? ORDER BY created_at, id').all(conversationId); }
  createConversation(title = 'New conversation') { const item = { id: id(), title, created_at: now(), updated_at: now() }; this.stmt('INSERT INTO conversations VALUES(?,?,?,?)').run(item.id,item.title,item.created_at,item.updated_at); return item; }
  touchConversation(id, title) { this.stmt('UPDATE conversations SET title=COALESCE(?,title),updated_at=? WHERE id=?').run(title || null, now(), id); }
  deleteConversation(id) { this.stmt('DELETE FROM messages WHERE conversation_id=?').run(id); return this.stmt('DELETE FROM conversations WHERE id=?').run(id).changes > 0; }
  addMessage(conversationId, role, content, provider = null, status = 'complete', origin = null) { const item = { id: id(), conversation_id: conversationId, role, content, provider, status, created_at: now(), origin }; this.stmt('INSERT INTO messages(id,conversation_id,role,content,provider,status,created_at,origin) VALUES(?,?,?,?,?,?,?,?)').run(item.id,item.conversation_id,item.role,item.content,item.provider,item.status,item.created_at,item.origin); this.touchConversation(conversationId); return item; }
  roots() { return this.stmt('SELECT * FROM workspace_roots ORDER BY added_at, id').all(); }
  addRoot(rootPath) { const item = { id: id(), path: rootPath, added_at: now() }; this.stmt('INSERT INTO workspace_roots VALUES(?,?,?)').run(item.id,item.path,item.added_at); return item; }
  removeRoot(id) { return this.stmt('DELETE FROM workspace_roots WHERE id=?').run(id).changes > 0; }
  workspaceEdits(status) {
    if (status) return this.stmt('SELECT * FROM workspace_edits WHERE status=? ORDER BY created_at DESC, id DESC').all(status);
    return this.stmt('SELECT * FROM workspace_edits ORDER BY created_at DESC, id DESC').all();
  }
  proposeWorkspaceEdit(filePath, content, reason = 'Proposed code edit or self-evolution update') {
    ...
    this.stmt('INSERT INTO workspace_edits VALUES(?,?,?,?,?,?,?)').run(item.id, item.file_path, item.content, item.reason, item.status, item.created_at, item.reviewed_at);
    return item;
  }
  workspaceEdit(id) {
    return this.stmt('SELECT * FROM workspace_edits WHERE id=?').get(id) || null;
  }
  ...
  updateWorkspaceEditStatus(id, status) {
    ...
    this.stmt('UPDATE workspace_edits SET status=?, reviewed_at=? WHERE id=?').run(status, now(), id);
    return this.workspaceEdit(id);
  }

  mcpServers() {
    const rows = this.stmt('SELECT * FROM mcp_servers ORDER BY created_at, id').all();
    return rows.map(r => ({
      id: r.id,
      name: r.name,
      type: r.type,
      ...
      tools: parseToolsJson(r.tools_json),
      createdAt: r.created_at,
      updatedAt: r.updated_at
    }));
  }
  mcpServer(id) {
    const r = this.stmt('SELECT * FROM mcp_servers WHERE id=?').get(id);
    if (!r) return null;
    return {
      id: r.id,
      name: r.name,
  ...
  addMcpServer({ name, type = 'http', endpoint, tools = [] }) {
    ...
    this.stmt('INSERT INTO mcp_servers(id,name,type,endpoint,status,latency_ms,last_probe_at,failure_reason,tools_json,created_at,updated_at) VALUES(?,?,?,?,?,NULL,NULL,NULL,?,?,?)')
  ...
  updateMcpServer(id, updates) {
    ...
    const updatedAt = now();
    this.stmt('UPDATE mcp_servers SET name=?, type=?, endpoint=?, status=?, latency_ms=?, tools_json=?, updated_at=? WHERE id=?')
  ...
  recordMcpProbe(id, { status, latencyMs = null, failureReason = null }) {
    this.stmt('UPDATE mcp_servers SET status=?, latency_ms=?, last_probe_at=?, failure_reason=?, updated_at=? WHERE id=?')
      .run(status, latencyMs, now(), status === 'connected' ? null : (failureReason || null)?.slice(0, 500) ?? null, now(), id);
    return this.mcpServer(id);
  }
  deleteMcpServer(id) {
    return this.stmt('DELETE FROM mcp_servers WHERE id=?').run(id).changes > 0;
  }

  skills() {
    const rows = this.stmt('SELECT * FROM skills ORDER BY type DESC, created_at DESC, id DESC').all();
    return rows.map(r => ({
      id: r.id,
      name: r.name,
  ...
  skill(id) {
    const r = this.stmt('SELECT * FROM skills WHERE id=?').get(id);
    if (!r) return null;
    return {
      id: r.id,
      name: r.name,
  ...
  skillByCommand(cmd) {
    ...
    const r = this.stmt('SELECT * FROM skills WHERE LOWER(slash_command)=?').get(normalized);
    if (!r) return null;
    return {
      id: r.id,
      name: r.name,
  ...
  addSkill({ name, slashCommand, description, code, enabled = true, type = 'custom', author = 'User', version = '1.0.0' }) {
    ...
    this.stmt('INSERT INTO skills(id,name,slash_command,description,code,enabled,type,author,version,created_at,updated_at,execution_provenance) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)')
  ...
  updateSkill(id, updates) {
    ...
    this.stmt('UPDATE skills SET name=?, slash_command=?, description=?, code=?, enabled=?, updated_at=?, execution_provenance=? WHERE id=?')
  ...
  deleteSkill(id) {
    return this.stmt('DELETE FROM skills WHERE id=?').run(id).changes > 0;
  }
  ...
  memories(category = null) {
    if (category && category !== 'all') {
      return this.stmt('SELECT * FROM memories WHERE category=? ORDER BY updated_at DESC, id DESC').all(category);
    }
    return this.stmt('SELECT * FROM memories ORDER BY updated_at DESC, id DESC').all();
  }
  memory(id) {
    return this.stmt('SELECT * FROM memories WHERE id=?').get(id) || null;
  }
  addMemory({ category = 'user_preference', key, value, importance = 'medium' }) {
    ...
    this.stmt('INSERT INTO memories(id,category,key,value,importance,created_at,updated_at) VALUES(?,?,?,?,?,?,?)')
  ...
  updateMemory(id, updates) {
    ...
    const updatedAt = now();
    this.stmt('UPDATE memories SET category=?, key=?, value=?, importance=?, updated_at=? WHERE id=?')
      .run(category, key, value, importance, updatedAt, id);
    return this.memory(id);
  }
  deleteMemory(id) {
    return this.stmt('DELETE FROM memories WHERE id=?').run(id).changes > 0;
  }
  ...
  createAgentRun({ conversation_id = null, agent_id, adapter, parent_run_id = null, mode = 'solo', objective = '', effective_capabilities = [] }) {
    ...
    this.stmt('INSERT INTO agent_runs (id, conversation_id, agent_id, adapter, parent_run_id, mode, status, objective, result, started_at, completed_at, effective_capabilities) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)')
  ...
  setAgentRunAuthority(runId, { adapter, effectiveCapabilities = [] }) {
    this.stmt('UPDATE agent_runs SET adapter=?, effective_capabilities=? WHERE id=?').run(adapter, JSON.stringify(effectiveCapabilities), runId);
  ...
  updateAgentRun(runId, updates) {
    ...
    this.stmt('UPDATE agent_runs SET status=?, result=?, completed_at=? WHERE id=?')
      .run(status, result, completedAt, runId);
    return this.agentRun(runId);
  }

  agentRun(runId) {
    return this.stmt('SELECT * FROM agent_runs WHERE id=?').get(runId) || null;
  }

  agentRuns(conversationId = null) {
    if (conversationId) {
      return this.stmt('SELECT * FROM agent_runs WHERE conversation_id=? ORDER BY started_at DESC, id DESC').all(conversationId);
    }
    return this.stmt('SELECT * FROM agent_runs ORDER BY started_at DESC, id DESC LIMIT 100').all();
  }
  ...
  providers() {
    return this.stmt('SELECT * FROM providers ORDER BY priority ASC, name ASC, id ASC').all()
      .map((row) => this._providerRow(row));
  }
  provider(id) {
    const row = this.stmt('SELECT * FROM providers WHERE id=?').get(id);
    return row ? this._providerRow(row) : null;
  }
  addProvider({ id, name, protocol, base_url, model = '', api_key, tags = [], enabled = true, priority = 50 }) {
    const item = {
      id: id || `prov-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`,
    ...
    this.stmt('INSERT INTO providers (id,name,protocol,base_url,model,api_key_enc,tags,enabled,priority,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
  ...
  updateProvider(id, updates) {
    ...
    this.stmt('UPDATE providers SET name=?,protocol=?,base_url=?,model=?,api_key_enc=?,tags=?,enabled=?,priority=?,updated_at=? WHERE id=?')
      .run(name, protocol, base_url, model, api_key_enc, tags, enabled, priority, updated_at, id);
    return this.provider(id);
  }
  deleteProvider(id) {
    return this.stmt('DELETE FROM providers WHERE id=?').run(id).changes > 0;
  }
  toggleProvider(id) {
    const existing = this.provider(id);
    if (!existing) return null;
  ...
  providerApiKey(id) {
    const row = this.stmt('SELECT api_key_enc FROM providers WHERE id=?').get(id);
  ...
  seedProvidersFromEnv() {
    const count = this.stmt('SELECT COUNT(*) as c FROM providers').get().c;
    if (count > 0) return;
    const seeds = [];
  ...
  _providerRow(row) {
    return {
      ...
      model: row.model,
      api_key_set: Boolean(row.api_key_enc),
      _api_key_enc: row.api_key_enc,
