...
const READ_MODELS = {
  ...
  recent_conversations: (db, limit) => db.prepare('SELECT id, title, created_at, updated_at FROM conversations ORDER BY updated_at DESC LIMIT ?').all(limit),
  skill_summary: (db, limit) => db.prepare('SELECT id, name, slash_command, description, enabled, type, version, execution_provenance FROM skills ORDER BY type DESC, created_at DESC LIMIT ?').all(limit),
  memory_summary: (db, limit) => db.prepare('SELECT id, category, key, value, importance, updated_at FROM memories ORDER BY updated_at DESC LIMIT ?').all(limit),
  agent_run_summary: (db, limit) => db.prepare('SELECT id, agent_id, adapter, mode, status, effective_capabilities, started_at, completed_at FROM agent_runs ORDER BY started_at DESC LIMIT ?').all(limit),
  mcp_server_summary: (db, limit) => db.prepare('SELECT id, name, type, status, latency_ms FROM mcp_servers ORDER BY created_at LIMIT ?').all(limit),
  workspace_roots: (db, limit) => db.prepare('SELECT id, path, added_at FROM workspace_roots ORDER BY added_at LIMIT ?').all(limit),
};
...
ALTER TABLE agent_runs_related RENAME TO agent_runs;`);
  ...
  authorizationAudit(limit = 100) {
    return this.db.prepare('SELECT * FROM authorization_audit ORDER BY created_at DESC LIMIT ?').all(limit);
  }
