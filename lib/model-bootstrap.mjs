...
export class VoiceModelBootstrap {
  constructor({ root = process.env.JARVIS_MODEL_DIR || path.resolve('models'), temporaryRoot = process.env.JARVIS_TEMP_DIR || path.resolve('cache', 'temp'), manifest = voiceModelManifest, publish = () => {} } = {}) { this.root = root; this.temporaryRoot = temporaryRoot; this.manifest = manifest; this.publish = publish; }
