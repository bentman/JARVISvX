...
// Anchor project-local defaults to this file's own location (<root>/lib/model-bootstrap.mjs),
// not process.cwd() — the daemon/CLI can be launched from any working directory.
...
export class VoiceModelBootstrap {
  constructor({ root = process.env.JARVIS_MODEL_DIR || path.join(PROJECT_ROOT, 'models'), temporaryRoot = process.env.JARVIS_TEMP_DIR || path.join(PROJECT_ROOT, 'cache', 'temp'), manifest = voiceModelManifest, publish = () => {} } = {}) { this.root = root; this.temporaryRoot = temporaryRoot; this.manifest = manifest; this.publish = publish; }
