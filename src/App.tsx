...
export default function App() {
  ...
  const send = async (event?: FormEvent, dictated?: string, origin: 'desktop-text' | 'voice' = 'desktop-text', targetConversationId?: string) => {
    ...
    try { await api.streamChat({ conversationId, content, providerId: activeProvider, model: selectedModel, allowCloud: cloudApproved, origin }, (message) => { if (message.type === 'start') conversationId = message.conversationId; if (origin === 'voice' && message.type === 'token') window.dispatchEvent(new CustomEvent('jarvis:speak', { detail: { type: 'assistant-token', value: message.value, conversationId: message.conversationId || conversationId } })); if (origin === 'voice' && message.type === 'turn-complete') window.dispatchEvent(new CustomEvent('jarvis:speak', { detail: { type: 'assistant-complete', conversationId: message.conversationId || conversationId } })); if (origin === 'voice' && (message.type === 'cancelled' || message.type === 'error')) window.dispatchEvent(new CustomEvent('jarvis:speak', { detail: { type: 'assistant-error', conversationId: message.conversationId || conversationId } })); if (message.type === 'token') { setCurrent((value) => value ? { ...value, id: conversationId || value.id, messages: value.messages?.map((item, index, all) => index === all.length - 1 ? { ...item, content: item.content + message.value } : item) } : value); } if (message.type === 'error') setError(message.message); }); if (conversationId) await selectConversation(conversationId); await refresh(); }
