...
const cleanVoiceTranscript = (text: string) => { const cleaned = String(text || '').replace(/^\s*(?:[\[(]?(?:blank_audio|blank audio|silence|no speech|music|inaudible|clicking|click|noise|wooshing(?: sound)?|water splashing|splashing|wind|breathing)[\])]?\.?)*\s*$/i, '').replace(/^\s*(?:(?:hey\s+)?(?:jarvis|elvis|travis)|age\s+of\s+(?:elvis|jarvis))\b[\s,.:;-]*/i, '').trim(); return cleaned && !/^\s*[\[(]?[a-z\s-]+(?:sound|noise|music|breathing|wind|splashing)[\])]?\.?\s*$/i.test(cleaned) ? cleaned : null; };
const mergeUnique = (items: string[]) => Array.from(new Set(items.filter(Boolean)));

export default function App() {
  const [providers, setProviders] = useState<Provider[]>([]);
  // null is Automatic: the request omits providerId and the daemon routes.
  const [providerChoice, setProviderChoice] = useState<string | null>(null);
  const [effectiveProvider, setEffectiveProvider] = useState<string | null>(null);
  const [routed, setRouted] = useState<{ provider: string; model: string; reason: string } | null>(null);
  ...
  const refreshProviders = useCallback(async () => {
    ...
    try {
      ...
      setEffectiveProvider(effective);
  ...
  useDaemonEvents((event) => {
    if (event.type === 'agent-run-complete' && event.conversationId) {
      void syncConversation(event.conversationId);
      return;
    }
    if (event.type === 'agent-start' && event.conversationId && event.conversationId === current?.id) {
      setCurrent((value) => {
        if (!value || value.id !== event.conversationId) return value;
        const msgId = `agent-${event.runId}-${event.agentId}`;
        const existing = value.messages?.find((m) => m.id === msgId);
        if (existing) return value;
        const newMsg = {
          id: msgId,
          role: 'assistant' as const,
          content: `**${event.speaker?.name || event.agentId} (${event.speaker?.voice || ''}):**\n`,
          status: 'streaming' as const,
          created_at: new Date().toISOString()
        };
        return { ...value, messages: [...(value.messages || []), newMsg] };
      });
      return;
    }
    if (event.type === 'agent-token' && event.conversationId && event.conversationId === current?.id) {
      setCurrent((value) => {
        if (!value || value.id !== event.conversationId) return value;
        const msgId = `agent-${event.runId}-${event.agentId}`;
        const msgs = value.messages || [];
        const hasMsg = msgs.some((m) => m.id === msgId);
        if (!hasMsg) {
          const newMsg = {
            id: msgId,
            role: 'assistant' as const,
            content: `**${event.speaker?.name || event.agentId}:**\n${event.value}`,
            status: 'streaming' as const,
            created_at: new Date().toISOString()
          };
          return { ...value, messages: [...msgs, newMsg] };
        }
        return {
          ...value,
          messages: msgs.map((m) => m.id === msgId ? { ...m, content: m.content + event.value } : m)
        };
      });
      return;
    }
    if (event.type === 'agent-turn-complete' && event.conversationId && event.conversationId === current?.id) {
      setCurrent((value) => {
        if (!value || value.id !== event.conversationId) return value;
        const msgId = `agent-${event.runId}-${event.agentId}`;
        return {
          ...value,
          messages: (value.messages || []).map((m) => m.id === msgId ? { ...m, status: 'complete' as const } : m)
        };
      });
      return;
    }
  ...
  // Automatic may still route to cloud, so the approval control stays available
  // whenever an eligible cloud provider is configured.
  ...
  const send = async (event?: FormEvent, dictated?: string, origin: 'desktop-text' | 'voice' = 'desktop-text', targetConversationId?: string | null) => {
    ...
    if (providerChoice !== null && !selectedModel && !content.startsWith('/')) { setError(`Select a model for ${activeProviderInfo?.label || 'the active provider'} before sending.`); return; }
    ...
    if (isCloudProvider && cloudApproved) approvalRequests.push(['provider.cloud', providerChoice ?? 'auto']);
    ...
    try {
      await api.streamChat({ conversationId, content, providerId: providerChoice ?? undefined, model: providerChoice === null ? undefined : selectedModel, approvals, origin }, (message) => {
