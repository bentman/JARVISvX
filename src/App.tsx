import { Activity, Bot, Brain, ChevronRight, CircleStop, Cloud, Cpu, FolderPlus, MessageSquarePlus, Mic, Send, Settings2, Trash2, Users, X, Zap } from 'lucide-react';
...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    try { const [providerData, history, rootData] = await Promise.all([api.providers(), api.conversations(), api.roots()]); const nextActiveProvider = providerData.settings.activeProvider; let nextProviders = providerData.providers; try { const modelData = await api.models(nextActiveProvider); if (modelData.models.length) nextProviders = nextProviders.map((provider) => provider.id === nextActiveProvider ? { ...provider, models: mergeUnique([...(provider.models || []), ...modelData.models]), available: true } : provider); } catch {} setProviders(nextProviders); setActiveProvider(nextActiveProvider); const nextActiveModels = nextProviders.find((provider) => provider.id === nextActiveProvider)?.models || []; if (providerData.settings.activeModel) setSelectedModel(providerData.settings.activeModel); else if (nextActiveModels[0]) setSelectedModel(nextActiveModels[0]); setConversations(history); setRoots(rootData); }
