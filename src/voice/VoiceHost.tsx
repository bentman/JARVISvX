  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try {
        ...
        const stopPlayback = () => { playbackGeneration.current += 1; audioQueue.current = []; playing.current = false; playback.current.forEach((output) => output.stop()); playback.current.clear(); void api.voiceEvent({ type: 'playback', state: 'interrupted', conversationId: speakingConversation.current }); };
        const playNext = () => {
        ...
        worker.current.onmessage = ({ data }) => {
          ...
          if (data.type === 'sentence-ready') void api.voiceEvent({ type: 'sentence-ready', text: data.text || null, conversationId: speakingConversation.current });
          if (data.type === 'audio') { stateRef.current('speaking'); audioQueue.current.push({ samples: new Float32Array(data.samples), rate: data.sampleRate }); playNext(); }
        ...
        const speak = (event: Event) => { const detail = (event as CustomEvent).detail; if (detail?.type === 'interrupt') { stopPlayback(); worker.current?.postMessage({ type: 'reset' }); return; } worker.current?.postMessage(detail); };
        window.addEventListener('jarvis:speak', speak);
        const eventController = new AbortController();
        void (async () => {
          try {
            for await (const event of api.events(eventController.signal)) {
              if (event.type === 'voice-state') { if (typeof event.voice === 'string') { stopPlayback(); worker.current?.postMessage({ type: 'voice', voice: event.voice }); } if (typeof event.enabled === 'boolean') { enabled.current = event.enabled; if (!enabled.current) stopPlayback(); worker.current?.postMessage({ type: 'listening', enabled: enabled.current }); } continue; }
              if (!enabled.current) continue;
              if (event.type === 'token') { speakingConversation.current = event.conversationId || null; worker.current?.postMessage({ type: 'speak', token: event.value, done: false }); continue; }
              if (event.type === 'turn-complete') { if (!event.conversationId || event.conversationId === speakingConversation.current) worker.current?.postMessage({ type: 'speak', token: '', done: true }); continue; }
        ...
        return () => { eventController.abort(); window.removeEventListener('jarvis:speak', speak); };
