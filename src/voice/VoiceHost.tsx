    ...
    const start = async () => {
      ...
      try {
        ...
        const stopEvents = subscribeEvents((event) => {
              ...
              if (event.type === 'agent-start') { if (sentence.current.trim()) { enqueue(sentence.current); sentence.current = ''; } return; }
              if (event.type === 'agent-token') { const nextConversation = event.conversationId || speakingConversation.current; if (nextConversation && speakingConversation.current && nextConversation !== speakingConversation.current) stopPlayback('New agent speech interrupted the previous output.'); if (!voiceTurnActive.current) pauseVoiceInput(); speakingConversation.current = nextConversation; speakingTurn.current = event.runId || speakingTurn.current; stateRef.current('speaking', `${event.speaker?.name || 'Agent'} speaking.`); queueSpeech(String(event.value || ''), false, event.speaker?.voice); return; }
              if (event.type === 'agent-turn-complete') { speakingConversation.current = event.conversationId || speakingConversation.current; speakingTurn.current = event.runId || speakingTurn.current; queueSpeech('', true, event.speaker?.voice); return; }
