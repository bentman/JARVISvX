...
function Tui({ client }) {
  const { exit } = useApp(); const [input, setInput] = useState(''); const [lines, setLines] = useState([]); const [conversation, setConversation] = useState(null); const [status, setStatus] = useState('connecting'); const [provider, setProvider] = useState('default'); const [model, setModel] = useState(''); const [voiceState, setVoiceState] = useState('connecting'); const [cloudApproved, setCloudApproved] = useState(false); const [agentApproved, setAgentApproved] = useState(false); const [toolsApproved, setToolsApproved] = useState(false); const activeTurn = useRef(null);
