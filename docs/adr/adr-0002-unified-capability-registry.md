...
Status: Implemented; partially superseded by [ADR 0003](adr-0003-authorization-context-and-execution-boundary.md)
Date: 2026-08-19
...
- ADR 0003 refines this baseline: skill provenance determines autonomous
  model invocation, and application-owned declarations classify MCP tool
  permissions.
...
- A mutating-capability grant authorizes model-callable capabilities classified
  as approval-required; delegating to a privileged agent via `agents_ask`
  requires a separate `agent.privileged` grant under ADR 0003.
