...
const { loadNpz } = createRequire(import.meta.url)('npyz');
...
let session; let styles; let loaded;
parentPort.on('message', async ({ id, modelPath, voicesPath, text, voice }) => {
  try { const samples = await synthesize(modelPath, voicesPath, text, voice); parentPort.postMessage({ id, ok: true, sampleRate: 24000, samples }, [samples.buffer]); }
...
async function synthesize(modelPath, voicesPath, text, voice) {
  ...
  if (loaded !== runtimeKey) { session = await ort.InferenceSession.create(modelPath); styles = await loadBundle(voicesPath); loaded = runtimeKey; }
  const voiceStyle = styles.get(voice); if (!voiceStyle) throw new Error(`Voice ${voice} is missing from voices-v1.0.bin.`);
  const phonemes = await espeak(String(text)); const tokens = [...phonemes].map((x) => V[x]).filter((x) => x !== undefined).slice(0, 510);
